...
foreach ($rResult as $aRow) {
    ...
    $deleteLink = admin_url(AFFILIATE_MANAGEMENT_MODULE_NAME . "/delete_affiliate/$id");
    ...
    if (has_permission(AFFILIATE_MANAGEMENT_MODULE_NAME, '', 'edit')) {
        ...
        if (!empty($statusLink))
            $options .= '<span>|</span><a href="' . $statusLink . '" class="' . $className . '" data-toggle="tooltip" data-original-title="' . $statusText . '"><i class="' . $statusIcon . '"></i></a>';

        $options .= '<span>|</span><a class="_delete text-danger" data-toggle="tooltip" data-original-title="' . _l('delete') . '" href="' . $deleteLink . '"><i class="fa fa-trash-can"></i></a>';
    }
