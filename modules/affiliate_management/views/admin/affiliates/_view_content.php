...
$can_update_affiliate_id = AffiliateManagementHelper::can_update_affiliate_id($affiliate);
$affiliate_slug_update_url = base_url((is_client_logged_in() ? 'clients/' : '') . AFFILIATE_MANAGEMENT_MODULE_NAME . '/update_affiliate_slug' . (is_client_logged_in() ? '' : '/' . $affiliate->affiliate_id));
?>
<div class="content affiliate-view">
    <div class="row">
        ...
        <div class="col-sm-6 col-md-3 tw-mb-2">
            <div
                class="tw-bg-white tw-px-6 tw-py-5 tw-pt-3 top_stats_wrapper tw-shadow-sm tw-rounded-lg tw-border tw-border-neutral-200 tw-border-solid top_stats_wrapper">
                    ...
                    <div class="tw-my-3">
                        ...
                        <div class="input-group tw-max-w-sm tw-mx-auto">
                            ...
                            <span class="input-group-btn">
                                <button class="btn btn-primary group-update _delete" id="edit-group-button"
                                    data-toggle="tooltip" data-title="<?= _l('save'); ?>" title="<?= _l('save'); ?>"><i
                                        class="fa fa-save"></i></button>
                            </span>
                        </div>
                        <?= form_close(); ?>
                    </div>
                    <a class="btn btn-primary" href="<?= admin_url('clients/login_as_client/' . $affiliate->userid); ?>"
                        target="_blank">
                    ...
                    <div class="tw-flex tw-flex-col tw-grow">
                        <div class="input-group">
                            <input type="text" class="form-control" id="affiliateLink"
                                value="<?= rtrim(base_url('register'), '/'); ?>" readonly>
                            <span class="input-group-addon">/</span>
                            <input type="text" class="form-control" id="slug-id"
                                data-value="<?= $affiliate->affiliate_slug; ?>"
                                data-action="<?= $affiliate_slug_update_url; ?>"
                                value="<?= $affiliate->affiliate_slug; ?>" readonly>
                            <span class="input-group-btn">
                                <?php if ($can_update_affiliate_id) : ?>
                                <button class="btn btn-default slug-update" id="edit-button" data-toggle="tooltip"
                                    data-title="<?= _l('edit'); ?>"><i class="fa fa-pen"></i></button>
                                <button class="btn btn-success slug-update" style="display: none;" id="save-button"
                                    data-toggle="tooltip" data-title="<?= _l('save'); ?>"><i
                                        class="fa fa-save"></i></button>
                                <?php endif; ?>
                        ...
                        <div class="tw-w-full text-right">
                            <small class="text-muted mt-2" id="copySuccessMessage"
                                style="display: none;"><?= _l('affiliate_management_link_copied'); ?></small>
                        </div>
                    </div>
                    <div>
                        <a href="<?= base_url(AFFILIATE_MANAGEMENT_MODULE_NAME . '/join/' . (is_admin() ? $affiliate->affiliate_id : '')); ?>"
                            target="_blank" data-toggle="tooltip" data-title="<?= _l('info'); ?>" type="button"
                            class="btn btn-info tw-rounded-full focus"><i class="fa fa-info"></i></a>
            ...
            <div class="panel_s">
                <div class="panel-body">
                    ...
                    <div class="text-center tw-flex tw-flex-col tw-mb-4">
                        <span
                            class="tw-text-2xl tw-font-bold text-success"><?= app_format_money($affiliate->balance, $currency); ?></span>
                    ...
                    <div class="text-right">
                        <button type="submit" data-loading-text="..." data-form="#affiliates_form"
                            class="btn btn-primary mtop15 mbot15"
                            <?php if (!is_client_logged_in() || $affiliate->status === AffiliateManagementHelper::STATUS_PENDING) echo 'disabled'; ?>><?php echo _l('affiliate_management_request_payout'); ?></button>
                    </div>
                    <?php echo form_close(); ?>
                </div>
            </div>
