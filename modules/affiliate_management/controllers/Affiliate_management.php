...
require_once __DIR__ . '/Affiliate_management_common_trait.php';

class Affiliate_management extends AdminController
{
    use Affiliate_management_common_trait;
    ...
    {
        // Check for permission
        if (!has_permission(AFFILIATE_MANAGEMENT_MODULE_NAME, '', 'delete')) {
            return access_denied(AFFILIATE_MANAGEMENT_MODULE_NAME);
        }

        $this->affiliate_management_model->delete_affiliate($affiliate_id);

        set_alert('success', _l('deleted', _l('affiliate_management_affiliate')));
     ...
     * @return void
     */
    public function update_affiliate_slug($affiliate_id)
    {
        $affiliate = $this->affiliate_management_model->get_affiliate($affiliate_id);

        return $this->_update_affiliate_slug($affiliate);
    }
}