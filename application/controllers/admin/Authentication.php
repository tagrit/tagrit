...
class Authentication extends App_Controller
{
    ...
    public function getCountryIdEmail($email)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($country_query->num_rows() > 0) {
                return $country_query->row()->country_id;
            } else {
                return null;
            }
        } else {
    ...
    public function admin()
    {
        ...
        if ($this->input->post('validate_country') == '1') {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                if ($this->input->post('validate_country') == '1') {
                    if ($this->getCountryIdEmail($email) != $this->input->post('country_id')) {
                        set_alert('danger', 'Please Select a Valid Country');
                ...
                if ($this->input->post('validate_country') == '1') {
                    ...
                    $this->db->delete(db_prefix() . '_courier_audit_logs');

                    $this->db->insert(db_prefix() . '_courier_audit_logs', [
                        'staff_id' => $staff_id,
