...
<div id="invoice-preview">
    <div class="row">
      ...
      if ($invoice->recurring > 0 || $invoice->is_recurring_from != null) {
        ...
        <div class="col-md-12">
         ...
         && $recurring_invoice->cycles == $recurring_invoice->total_cycles) { ?>
                <div class="alert alert-info no-mbot">
                    <?= e(_l('recurring_has_ended', _l('invoice_lowercase'))); ?>
                </div>
               ...
               if ($recurring_invoice->status == Invoices_model::STATUS_DRAFT) {
                   echo '<i class="fa-solid fa-circle-exclamation fa-fw text-warning tw-mr-1" data-toggle="tooltip" title="' . _l('recurring_invoice_draft_notice') . '"></i>';
                    ...
                    <b>
                        <?= e($recurring_invoice->cycles == 0 ? _l('cycles_infinity') : $recurring_invoice->cycles - $recurring_invoice->total_cycles);
                    ?>
                    </b>
                </span>
                <?php
            ...
            <?php if ($invoice->is_recurring_from != null) { ?>
            <?= '<p class="text-muted' . ($show_recurring_invoice_info ? ' mtop15' : '') . '">' . _l('invoice_recurring_from', '<a href="' . admin_url('invoices/list_invoices/' . $invoice->is_recurring_from) . '" onclick="init_invoice(' . $invoice->is_recurring_from . ');return false;">' . e(format_invoice_number($invoice->is_recurring_from)) . '</a></p>'); ?>
            <?php } ?>
        </div>
        ...
        <div class="col-md-12">
            <h4 class="font-medium mtop15 mbot20"><?= _l('related_to_project', [
        ...
        <div class="col-md-6 col-sm-6">
            <h4 class="bold">
                <?php $tags = get_tags_in($invoice->id, 'invoice'); ?>
                <?php if (count($tags) > 0) { ?>
                <i class="fa fa-tag" aria-hidden="true" data-toggle="tooltip"
                    data-title="<?= e(implode(', ', $tags)); ?>"></i>
                <?php } ?>
                <a
                    href="<?= admin_url('invoices/invoice/' . $invoice->id); ?>">
        ...
        <div class="col-sm-6 text-right">
            <span
                class="bold"><?= _l('invoice_bill_to'); ?></span>
            ...
            <p class="no-mbot">
                <span
                    class="bold"><?= _l('sale_agent_string'); ?>:
                </span>
            ...
            <table class="table text-right">
                <tbody>
                    <tr id="subtotal">
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('invoice_subtotal'); ?></span>
                        </td>
                        <td class="subtotal">
                    ...
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('invoice_discount'); ?>
                    ...
                    <?php foreach ($items->taxes() as $tax) {
                        echo '<tr class="tax-area"><td class="tw-font-medium !tw-text-neutral-700">' . e($tax['taxname']) . ' (' . e(app_format_number($tax['taxrate'])) . '%)</td><td>' . e(app_format_money($tax['total_tax'], $invoice->currency_name)) . '</td></tr>';
                    } ?>
                    <?php if ((int) $invoice->adjustment != 0) { ?>
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('invoice_adjustment'); ?></span>
                        </td>
                        <td class="adjustment">
                    ...
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('invoice_total'); ?></span>
                        </td>
                        <td class="total">
                    ...
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('invoice_total_paid'); ?></span>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium tw-text-neutral-700"><?= _l('applied_credits'); ?></span>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td>
                            <span
                                class="tw-font-medium<?= $invoice->total_left_to_pay > 0 ? ' text-danger ' : ''; ?>">
