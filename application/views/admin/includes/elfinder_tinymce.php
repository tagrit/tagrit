...
<head>
    ...
    <script
        src="<?= base_url('assets/plugins/jquery-ui/jquery-ui.min.js'); ?>">
    </script>
    <!-- elFinder JS (REQUIRED) -->
    <script
        src="<?= base_url('assets/plugins/elFinder/js/elfinder.min.js?v=' . get_app_version()); ?>">
    </script>
    <?= app_compile_css('editor-media');

if ($mediaLocale != 'en' && file_exists(FCPATH . 'assets/plugins/elFinder/js/i18n/elfinder.' . $mediaLocale . '.js')) { ?>
    <script
        src="<?= base_url('assets/plugins/elFinder/js/i18n/elfinder.' . $mediaLocale . '.js?v=' . get_app_version()); ?>">
    </script>
    <?php } ?>
...
<body>
    ...
    <link rel="stylesheet" type="text/css"
        href="<?= base_url('assets/plugins/elFinder/themes/Material/css/theme-gray.css?v=' . get_app_version()); ?>">
    ...
    <script>
        define('elFinderConfig', {
            ...
            defaultOpts: {
                ...
                url: '<?= $connector . '?editor=true' ?>', // connector URL (REQUIRED)
                commandsOptions: {
                    edit: {
                        extraOptions: {
        ...
        (function() {
            var // elFinder version
                ...
                start = function(elFinder, editors, config) {
                    // load jQueryUI CSS
                    elFinder.prototype.loadCss('//cdnjs.cloudflare.com/ajax/libs/jqueryui/' + uiver +
                    ...
                    $(function() {
                        ...
                        if (config && config.managers) {
                            $.each(config.managers, function(id, mOpts) {
                                ...
                                try {
                                    mOpts.commandsOptions.edit.editors = mOpts.commandsOptions.edit
                ...
                ie8 = (typeof window.addEventListener === 'undefined' && typeof document.getElementsByClassName ===
                    'undefined');
            ...
            require.config({
                ...
                paths: {
                    'jquery': '//cdnjs.cloudflare.com/ajax/libs/jquery/' + (ie8 ? '1.12.4' : jqver) +
                        '/jquery.min',
