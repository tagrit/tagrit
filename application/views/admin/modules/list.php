...
<div id="wrapper">
    <div class="content">
        <div class="tw-mb-8">
            <?= form_open_multipart(admin_url('modules/upload'), ['id' => 'module_install_form', 'class' => 'sm:flex sm:items-center']); ?>
            <h3 class="tw-mb-2 tw-mt-0 tw-text-lg tw-leading-6 tw-text-neutral-900 tw-font-bold">Upload Module</h3>
                        ...
                        <div class="table-responsive">
                            <table class="table dt-table" data-order-type="asc" data-order-col="0">
                                ...
                                <tbody>
                                    ...
                                    <tr
                                        class="<?= $module['activated'] === 1 && ! $database_upgrade_is_required ? 'info' : ''; ?><?= $database_upgrade_is_required ? ' warning' : ''; ?>">
                                        <td
                                        ...
                                        if ($newVersionData = $this->app_modules->new_version_available($system_name)) {
                                            ...
                                            if (isset($newVersionData['changelog']) && ! empty($newVersionData['changelog'])) {
                                                $version_actions[] = '<a href="' . $newVersionData['changelog'] . '" target="_blank" class="alert-link">Release Notes (' . $newVersionData['version'] . ')</a>';
                                            }

                                            if ($this->app_modules->is_update_handler_available($system_name)) {
                                                $version_actions[] = '<a href="' . admin_url('modules/update_version/' . $system_name) . '" id="update-module-' . $system_name . '" class="alert-link">Update</a>';
                                            }
