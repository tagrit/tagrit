        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">
                    <span
                        class="edit-title"><?= _l('payment_mode_edit_heading'); ?></span>
                    <span
                        class="add-title"><?= _l('payment_mode_add_heading'); ?></span>
                </h4>
            </div>
            <?= form_open('admin/paymentmodes/manage', ['id' => 'payment_modes_form']); ?>
            <?= form_hidden('paymentmodeid'); ?>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <?= render_input('name', 'payment_mode_add_edit_name'); ?>
                        <?= render_textarea('description', 'payment_mode_add_edit_description', '', ['data-toggle' => 'tooltip', 'title' => 'payment_mode_add_edit_description_tooltip', 'rows' => 5]); ?>
                        <div class="checkbox checkbox-primary">
                            <input type="checkbox" name="active" id="active">
                            <label
                                for="active"><?= _l('payment_mode_add_edit_active'); ?></label>
                        </div>
                        <div class="checkbox checkbox-primary">
                            <input type="checkbox" name="show_on_pdf" id="show_on_pdf">
                            <label
                                for="show_on_pdf"><?= e(_l('show_on_invoice_on_pdf', _l('payment_mode_add_edit_description'))); ?></label>
                        </div>
                        <div class="checkbox checkbox-primary">
                            <input type="checkbox" name="selected_by_default" id="selected_by_default">
                            <label
                                for="selected_by_default"><?= _l('settings_paymentmethod_default_selected_on_invoice'); ?></label>
                        </div>
                        <hr />
                        <div class="checkbox checkbox-primary pm-available-to">
                            <input type="checkbox" name="invoices_only" id="invoices_only">
                            <label
                                for="invoices_only"><?= _l('payment_mode_invoices_only'); ?></label>
                        </div>
                        <div class="checkbox checkbox-primary pm-available-to">
                            <input type="checkbox" name="expenses_only" id="expenses_only">
                            <label
                                for="expenses_only"><?= _l('payment_mode_expenses_only'); ?></label>
                        </div>
...
<script>
    window.addEventListener('load', function() {
        ...
        $('#payment_mode_modal').on('show.bs.modal', function(event) {
            ...
            $('#payment_mode_modal input[name="expenses_only"]').prop('checked', false).prop('disabled',
                false);
            $('#payment_mode_modal input[name="invoices_only"]').prop('checked', false).prop('disabled',
                false);
            ...
            if (typeof(id) !== 'undefined') {
                ...
                $('#payment_mode_modal input[name="expenses_only"]').prop('checked', expenses_only)
                    .change();
                $('#payment_mode_modal input[name="invoices_only"]').prop('checked', invoices_only)
                    .change();
                $('#payment_mode_modal input[name="show_on_pdf"]').prop('checked', show_on_pdf);
                $('#payment_mode_modal input[name="selected_by_default"]').prop('checked',
                    selected_by_default);
                ...
                $('#payment_mode_modal textarea[name="description"]').val(description.trim().replace(
                    /(<|&lt;)br\s*\/*(>|&gt;)/g, " "));
            }
        });
    });
