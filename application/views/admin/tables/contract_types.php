...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'name') {
            $_data = '<a href="#" class="tw-font-medium" onclick="edit_type(this,' . $aRow['id'] . '); return false;" data-name="' . e($aRow['name']) . '">' . e($_data) . '</a> <span class="badge pull-right">' . total_rows(db_prefix() . 'contracts', ['contract_type' => $aRow['id']]) . '</span>';
        }
        $row[] = $_data;
    }
